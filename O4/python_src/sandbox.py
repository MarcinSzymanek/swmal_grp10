...
def load_data(path_to_csv : str) -> pd.DataFrame:
    data_breast_cancer = pd.read_csv(path_to_csv)
...
if __name__ == "__main__":
    data_bc = load_data("breast_cancer_win/data.csv")
    data_bc.drop(columns=["Unnamed: 32", "id"], inplace=True)
    features = data_bc.columns
